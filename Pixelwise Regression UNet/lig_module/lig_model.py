...
from model.unet.unet import UNet2D
...
class LitModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx: int):
        ...
        """if self.current_epoch % 2 == 0 and batch_idx == 0:
            plt.imshow(inputs[batch_idx, 1, :,:], cmap = "gray")
            plt.show()"""
    ...
    def validation_step(self, batch, batch_idx: int):
        ...
        print("Input shape check: ", inputs.shape)
        # an inverse to the sigmoid function that limits values between 0 and 1 across the Y-axis
        logits = self(inputs)
        loss = self.criterion(logits.view(-1), targets.view(-1))
        
        """plt.title("validation_step output")
        plt.imshow(logits[0, 0, :, :], cmap = "gray")
        plt.show()"""
        ...
        print("Saving Predicts...")
        plt.axis('off')
        plt.imshow(predicts[:, :], cmap = "gray")
        plt.savefig(f"Output/epoch{self.current_epoch}batch{batch_idx}.png", bbox_inches='tight',transparent=True, pad_inches=0)

        mse = np.square(np.subtract(targets, predicts)).mean() # mean squared error
        ssim_ = ssim(targets, predicts, data_range=predicts.max() - predicts.min()) # structural similarity loss
        psnr_ = psnr(targets, predicts, data_range=predicts.max() - predicts.min()) # Peak Signal to Noise Ratio
        ...
        #pred_255[liver_mask] = 0
        #targ_255[liver_mask] = 0

        diff_255 = np.absolute(pred_255.ravel() - targ_255.ravel())
        mae = np.mean(diff_255) # mean absolute error 
    ...
    def test_step(self, batch, batch_idx: int):
        ...
        liver_mask = inputs == inputs[0][0][0]
        ...
        diff_255_mask = np.absolute(pred_255[~liver_mask].ravel() - targ_255[~liver_mask].ravel())
        mae_mask = np.mean(diff_255_mask)
